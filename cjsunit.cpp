...
namespace clib {
    ...
    unit_rule &unit_rule::set_attr(uint32_t attr) {
        this->attr = attr;
        return *this;
    }
    ...
    unit &cjsunit::token(const lexer_t &type) {
        return (*(unit_token *) nodes.alloc(sizeof(unit_token))).set_type(type).set_t(u_token).init(this);
    }
    ...
    unit_collection &cjsunit::append(unit *collection, unit *child) {
    ...
    unit_collection &cjsunit::merge(unit *a, unit *b) {
    ...
    unit_collection &cjsunit::collection(unit *a, unit *b, unit_t type) {
        a = copy(a);
        b = copy(b);
        a->next = a->prev = b;
        b->next = b->prev = a;
    ...
    nga_edge *cjsunit::conv_nga(unit *u) {
        if (u == nullptr)
            return nullptr;
    ...
    nga_edge *cjsunit::enga(unit *node, bool init) {
        auto _enga = (nga_edge *) nodes.alloc(sizeof(nga_edge));
        _enga->data = nullptr;
        _enga->skip = false;
        _enga->marked = false;
        if (init) {
    ...
    nga_edge *cjsunit::enga(unit *node, unit *u) {
        auto begin = status();
        auto end = status();
    ...
    nga_edge *cjsunit::connect(nga_status *a, nga_status *b, bool is_pda) {
        auto new_edge = is_pda ? (pda_edge *) nodes.alloc(sizeof(pda_edge))
                               : (nga_edge *) nodes.alloc(sizeof(nga_edge));
        new_edge->begin = a;
        new_edge->end = b;
    ...
    void cjsunit::disconnect(nga_status *status) {
    ...
    nga_status *cjsunit::status() {
        auto _status = (nga_status *) nodes.alloc(sizeof(nga_status));
        _status->final = false;
    ...
    pda_status *cjsunit::status(const char *label, int rule, bool final) {
        auto _status = (pda_status *) nodes.alloc(sizeof(pda_status));
        _status->label = label;
        _status->final = final;
        _status->in = _status->out = nullptr;
        _status->rule = rule;
    ...
    void cjsunit::add_edge(nga_edge_list *&list, nga_edge *edge) {
        if (list == nullptr) {
            list = (nga_edge_list *) nodes.alloc(sizeof(nga_edge_list));
            list->edge = edge;
            list->prev = list->next = list;
        } else {
            auto new_edge = (nga_edge_list *) nodes.alloc(sizeof(nga_edge_list));
            new_edge->edge = edge;
    ...
    void cjsunit::remove_edge(nga_edge_list *&list, nga_edge_list *edge) {
        if (list == nullptr) {
    ...
    const char *cjsunit::label(unit *focused, bool front) {
        std::stringstream ss;
    ...
    void cjsunit::label(unit *node, unit *parent, unit *focused, bool front, std::ostream &os) {
        if (node == nullptr)
            return;
    ...
    std::string cjsunit::print_unit(unit *u) {
        std::stringstream ss;
        print(u, nullptr, ss);
        return ss.str();
    }
    ...
    nga_status *cjsunit::delete_epsilon(nga_edge *edge) {
        edge->end->final = true;
    ...
    bool is_left_resursive_edge(nga_edge *edge, unit *rule) {
        return !edge->begin->in && edge->data &&
               edge->data->t == u_rule_ref && to_ref(edge->data)->child == rule;
    }

    void cjsunit::gen_pda(unit *root) {
        auto size = rules.size();
